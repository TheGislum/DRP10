...
for beta in [1, 0.1, 0.01, 0.001, 0]:
    for senario, max_sig, min_sig in senario_max_min:
        print("-------------------")
        print(
            f"python MUSE-XAE/MUSE_XAE.py --dataset Scenario_{senario} --augmentation 20 --max_sig {max_sig} --min_sig {min_sig} --iter 5 --n_jobs 20 --beta_v {beta} --directory beta_{beta}"
        )
        print("-------------------")
