...
class MUSE_XVAE(keras.Model):
    def __init__(self, input_dim=96, z=17, beta_v=1e-5, beta_r=0.001, Kx=10, **kwargs):
    ...
    def build_encoder(self):
        ...
        signatures = Sampling(K=self.Kx, name="encoder_layer")([z_mean, z_log_var])
    ...
    def test_step(self, data):
        ...
        kl_loss = -0.5 * (
            1
            + z_log_var
            - tf.math.log(self.var_p)
            - ((tf.square(z_mean) + tf.exp(z_log_var)) / self.var_p)
        )
